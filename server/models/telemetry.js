...
const Telemetry = {
  ...
  */
  ...
  client: function () {
    // Privacy-first: Only enable telemetry if explicitly opted in
    if (process.env.ENABLE_TELEMETRY !== "true" || this.isDev()) return null;
    const { PostHog } = require("posthog-node");
    return new PostHog(this.pubkey);
  },

  runtime: function () {
    if (process.env.SHROUD_LLM_RUNTIME === "docker") return "docker";
